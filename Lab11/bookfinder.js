...
function updateBookshelfView() {
    ...
    Object.keys(bookshelf).forEach(bookId => {
        ...
        bookItem.innerHTML = `
            ...
            <button id="toggle-shelf-${bookId}" onclick="toggleShelfDetails('${bookId}')">Expand Details</button>
            <div id="shelf-details-${bookId}" class="book-details" style="display:none;">
                <p>${book.volumeInfo.description || 'No description available.'}</p>
            </div>
        `;
...
function toggleShelfDetails(bookId) {
    const detailsDiv = document.getElementById('shelf-details-' + bookId);
    const toggleButton = document.getElementById('toggle-shelf-' + bookId);
    const isExpanded = detailsDiv.style.display === 'block';
